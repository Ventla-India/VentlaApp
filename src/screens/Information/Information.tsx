...
import GenericFlatList from '../../components/GenericFlatList';
  ...
  const Information = () => {
    const [customCategoriesData, setCustomCategoriesData] = useState<CategoryItem[]>([]);
    ...
    const fetchCustomCategories = async () => {
      ...
      try {
        ...
        const allCategories = realmService.getAll();
        setCustomCategoriesData(allCategories);
        ...
        try {
          ...
          if (offlineData.length > 0) {
            setCustomCategoriesData(offlineData);
    ...
    const fetchCustomCategoriesOnce = useCallback(async () => {
      try {
        // Get local data using RealmService
        const localData = realmService.getAll();
  
        if (localData.length > 0) {
          setCustomCategoriesData(localData);
          setLoading(false);
        } else {
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
          ...
          <GenericFlatList
            data={customCategoriesData.slice(0, 2)}
            horizontal
          ...
          {loading ? (
            ...
            <GenericFlatList
              ...
              ListEmptyComponent={<Text style={styles.emptyText}>No customCategoriesData found.</Text>}
              loading={loading}
